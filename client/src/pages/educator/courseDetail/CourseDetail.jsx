...
const CourseDetail = () => {
    ...
    const fetchData = async () => {
      try {
        const [courseRes, chaptersRes, lessonsRes] = await Promise.all([
          axios.get(`${proxy}/courses/get-one/${courseId}`),
          axios.get(`${proxy}/chapters/get-all/${courseId}`),
        ]);
  ...
  const addChapter = () => {
    ...
    const newChapter = async () => {
      ...
      setUpdating(true);
      try {
        await axios.post(`${proxy}/chapters/create`, chapter);
        await sleep(500);
      } catch (error) {
      } finally {
        setUpdating(false);
  ...
  const updateCourse = async () => {
    setUpdating(true);
    try {
      await axios.patch(`${proxy}/courses/${courseId}`, formCourse);
    } catch (error) {
    } finally {
      await sleep(1000);
      setUpdating(false);
  ...
  const handleRemove = async () => {
    ...
    if (res) {
      setUpdating(true);
      await axios.delete(`${proxy}/courses/${courseId}`);
      await sleep(1000);
      setUpdating(false);
